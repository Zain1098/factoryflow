...
part 'production_entry_header.dart';
...
class _ProductionScreenState extends ConsumerState<ProductionScreen> {
        ...
        children: [
          ...
          Card(
            ...
            child: Padding(
              padding: const EdgeInsets.all(16),
              child: Column(
                // RecordDateTimePicker contains an Expanded child. Stretching
                // this form column gives it a finite width and prevents the
                // repeated "BoxConstraints forces an infinite width" error.
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  Row(
          ...
          if (_partId != null) ...[
            ...
            rawMaterialAsync!.when(
              loading: () => const LinearProgressIndicator(),
              error: (error, _) =>
                  ErrorBanner('Unable to check available stock: $error'),
              data: (rawQty) => Container(
                padding: const EdgeInsets.all(14),
                decoration: BoxDecoration(
                  color: (_wipBatchNumber != null
                          ? Colors.amber
                          : rawQty > 0
                              ? Colors.teal
                              : Colors.red)
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        _wipBatchNumber != null
                            ? 'WIP continuation ready • ${_wipLastGoodQty?.toInt() ?? 0} PCS available from the previous stage'
                            : rawQty > 0
            ...
            children: [
              ...
              Expanded(
                ...
                child: FilledButton.icon(
                  onPressed:
                      _isSaving || _sessionEntries.isEmpty ? null : _saveAll,
                  icon: _isSaving
                      ? const SizedBox(
                          height: 18,
                          width: 18,
  ...
  Widget _buildSessionSummaryCard(ThemeData theme, ProductionFlowConfig flow) {
    ...
    final totalRejects =
        _sessionEntries.fold(0.0, (sum, e) => sum + e.rejectQty).toInt();
